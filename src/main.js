...
function create_world(){
    ...
    for(let i = 0; i < level_objects.length; i++){
        ...
        let obj;
        if("r" in item){
            obj = new ShapeCircle(item.x, item.y, item.r/2, {friction: 0.9,restitution: 0.1, isStatic: true});
        } else {
            obj = new ShapeRect(item.x, item.y, item.w, item.h, {friction: 0.9,restitution: 0.1, isStatic: true, angle: item.angle});
